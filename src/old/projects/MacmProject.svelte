...
>
  ...
  <HalfMdHalfImageCell
    md="During some downtime at work, I explored a Cisco research project focused on agentic AI workflows to improve performance on math problem-solving benchmarks. 
...
They also try and give the LLMs roles, with a judge, executor, and ideator, but I don't think this matters."
  ...
  <MdCell
    md="My goal with this project was just to learn, and mess around, using what they have as a baseline. Here is a rough timeline of what I attempted:
...
class AsyncLogger:
    ...
    />
    <CodeCell
      code={`#utils/gpt_async_assistant.py
...
async def agenerate_from_code_assistant(persona, content, assistant, thread):
  ...
  <MdCell
    md="*At this point it would have been a good idea to re-run benchmarks. I did not do this. This led to having a hard time measuring improvement, since my only comparison was their original code, which performs better since but uses expensive GPT and drew many more samples.* 
...
+        )
  ...
  <MdCell
    md="3. I noticed that the conditions as they were were not really conditions. They were basically just tyring to solve the problem. Half the time they would be steps to solve the problem. So basically just *drawing more samples* again. To do something different I wanted to do something more in order. When solving a math problem, I'd be doing different steps each time. I completely switched up the idea =>
...
I experimented with generating a set amount of steps and having the LLM 'decide'"
  ...
  <Section title="Code:" inset={true} hideToStart={true}>
    <MdCell md="All snippets taken from notebook2.ipynb" />
    <CodeCell
      code={`async def step_by_step_loop(question: str, log_path:str):
    ...
    />
    <CodeCell
      code={`async def step_by_step_loop_2(question: str, log_path:str):
    AsyncLogger.add_message(log_path, f"Starting run {log_path}",)
  ...
  <MdCell
    md="This was working okay. The problem is things start getting real complex. For that second step by step function
...
4. The overhead that comes with trying to improve chained LLM performance - there are so many variables to adjust - models, prompts, chaining order, and much more. And it's not a consistent result. "
  ...
  <MdCell
    md="I also had some more abstract rambling:
