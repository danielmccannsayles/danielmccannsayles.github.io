...
>
  <div slot="hiddenBlurb">
    <Teaser
      text="A game exploring using an LLM to arbitrate a fight with words. Sort of like when you were a kid, and would have an imaginary fight where you one-upped the other person"
    />
  </div>
  <MdCell
...
This also led me to decouple the display logic and the game logic, and cleared up a lot. Instead of having all these pygame classes that handled their state and display, I have all that logic in python classes on the server. I communicate the board state through a json object, and have pygame classes on the frontend/client. You can see below the seperation of logic - the client cares about looks, and the server cares about functionality.`}
  ...
  <Section title="Code:" inset={true} hideToStart={true}>
    <CodeCell
      code={`# Client implementation of Board
class Board:
    ...
    <CodeCell
      code={`# Server implementation of Board
class Board:
  ...
  <MdCell
    md={`They communicate through a simple network protocol, using sockets.
...
class ServerGame:
  ...
  <MdCell
    md={`The original idea of this game was to explore the added creativity an LLM could bring. In this case, the idea was to have an impartial judge add or subtract to bonuses to characters depending on the existing characters on the board.
...
> simplify prototypes & identify measurable objectives! BEFORE starting `}
  ...
  <Chips items={["LLMs", "async", "any.io", "python", "OOP"]} />
</Section>
