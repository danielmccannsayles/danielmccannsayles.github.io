export const llmVerbalAssistant = {
  ...
  content: [
    {
      type: "md",
      content: `I think well when walking, and I'd like to be able to brainstorm ideas whilst in this flow state. Going back for my computer or pulling out my phone  interrupts what I'm thinking. I wanted voice transcribed notes.
...
1. A basic iOS app that sends audio to a Node server on my computer.`,
    ...
    {
      type: "md",
      content: `2. The Node server transcribes the audio and listens for a wake word.`,
    },
    {
      type: "code",
      contents: [
...
app.get('/startSession', async (_, res) => {
    ...
    if (response.connected) {
      ...
      res.status(400).send(\`Error starting session: \${response.error}\`)
        ...
        {
          language: "typescript",
          content: `//helpers/rev-streaming.ts
...
export function startStreamingSession (revClient: RevAiStreamingClient | null, revAiStream: Duplex | null, bufferStream: PassThrough | null) {
    ...
    return connectedPromise
        ...
        {
          language: "typescript",
          content: `//processing/text_processing.ts
...
export function processTextListener (
  ...
  revAiStream.on('close', (err: any) => {})
    ...
    {
      type: "md",
      content: `3. Upon this wake word a call to ChatGPT is triggered with the past \`x\` amount of context, and a response is generated.`,
    },
    {
      type: "code",
      contents: [
  ...
  //.. part of wake function
    ...
    {
      type: "md",
      content: `4. The response is turned into audio, and sent to the app. The conversation transcription & response are stored as notes`,
    },
    {
      type: "code",
      contents: [
        {
          language: "typescript",
          content: `//llm_stuff/text_to_speech.ts
...
export function textStreamToAudioStream (textStream: Readable): Readable {
    ...
    {
      type: "md",
      content: `Some things I considered doing but didn't get around to were:
